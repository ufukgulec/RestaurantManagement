...
namespace RestaurantManagement.API.Controllers
{
    ...
    public class OrderTypeController : BaseController
    {
        ...
        {
            var result = await service.OrderTypeRepository.GetByIdAsync(id);
        ...
        {
            var result = await service.OrderTypeRepository.GetListAsync(default, false);
        ...
        [HttpPost("Add")]
        public async Task<IActionResult> Add(OrderType? entity)
        {
            var result = false;
            var Message = "";
            if (entity != null)
            {
                var exist = await service.OrderTypeRepository.GetSingleAsync(x => x.Name.ToLower() == entity.Name.ToLower());

                if (exist == null)
                {
                    result = await service.OrderTypeRepository.AddAsync(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin")]
        [HttpPost("Update")]
        public async Task<IActionResult> Update(OrderType? entity)
        {
            var result = false;
            var Message = "";
            if (entity != null)
            {
                var exist = await service.OrderTypeRepository.GetByIdAsync(entity.Id.ToString(), false);

                if (exist != null)
                {
                    result = await service.OrderTypeRepository.Update(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        public async Task<IActionResult> Remove(string id)
        {
            var result = false;
            var Message = "";
            var exist = await service.OrderTypeRepository.GetByIdAsync(id);
            ...
            if (exist != null)
            {
                ...
                {
                    exist.Active = false;
                    await service.OrderTypeRepository.Update(exist);
                    Message = "Kategori Pasif duruma getirildi.";
                }
                else
                {
                    result = await service.OrderTypeRepository.Remove(id);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
