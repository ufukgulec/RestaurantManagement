...
namespace RestaurantManagement.API.Controllers
{
    ...
    public class RoleController : BaseController
    {
        ...
        {
            var result = await service.RoleRepository.GetByIdAsync(id);
        ...
        {
            var result = await service.RoleRepository.GetListAsync(default, false);
        ...
        public async Task<IActionResult> Add(Role? entity)
        {
            ...
            if (entity != null)
            {
                var exist = await service.RoleRepository.GetSingleAsync(x => x.Name.ToLower() == entity.Name.ToLower());

                if (exist == null)
                {
                    result = await service.RoleRepository.AddAsync(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        public async Task<IActionResult> Update(Role? entity)
        {
            ...
            if (entity != null)
            {
                var exist = await service.RoleRepository.GetByIdAsync(entity.Id.ToString(), false);

                if (exist != null)
                {
                    result = await service.RoleRepository.Update(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        public async Task<IActionResult> Remove(string id)
        {
            var result = false;
            var Message = "";
            var exist = await service.RoleRepository.GetByIdAsync(id);
            ...
            if (exist != null)
            {
                ...
                {
                    exist.Active = false;
                    await service.RoleRepository.Update(exist);
                    Message = "Kategori Pasif duruma getirildi.";
                }
                else
                {
                    result = await service.RoleRepository.Remove(id);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
