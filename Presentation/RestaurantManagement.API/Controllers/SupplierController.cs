...
namespace RestaurantManagement.API.Controllers
{
    ...
    public class SupplierController : BaseController
    {
        ...
        {
            var result = await service.SupplierRepository.GetByIdAsync(id);
        ...
        {
            var result = await service.SupplierRepository.GetListAsync(default, false);
        ...
        public async Task<IActionResult> Add(Supplier? entity)
        {
            ...
            if (entity != null)
            {
                var exist = await service.SupplierRepository.GetSingleAsync(x => x.FullName.ToLower() == entity.FullName.ToLower());

                if (exist == null)
                {
                    result = await service.SupplierRepository.AddAsync(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        public async Task<IActionResult> Update(Supplier? entity)
        {
            ...
            if (entity != null)
            {
                var exist = await service.SupplierRepository.GetByIdAsync(entity.Id.ToString(), false);

                if (exist != null)
                {
                    result = await service.SupplierRepository.Update(entity);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
        ...
        public async Task<IActionResult> Remove(string id)
        {
            var result = false;
            var Message = "";
            var exist = await service.SupplierRepository.GetByIdAsync(id);
            ...
            if (exist != null)
            {
                ...
                {
                    exist.Active = false;
                    await service.SupplierRepository.Update(exist);
                    Message = "Kategori Pasif duruma getirildi.";
                }
                else
                {
                    result = await service.SupplierRepository.Remove(id);
                    if (result)
                    {
                        Message = "Başarılı";
                    }
